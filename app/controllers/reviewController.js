...
let reviewController={
	...
	editReview: function(req,res){
		var review_user_id = req.user._id;
			...
			Review.findOne({_id:review_id}, function(err, review) {
		        if(err){
			        ...
			        if(!review){
			            ...
			            if(!review.user_id.equals(review_user_id)){
  ...
  clientViewReviews: function (req, res) {
	...
	viewMyReviews: function (req, res) { //added
		...
		var reviews = Review.find(conditions).populate( {path: 'business_id', populate: {path: 'user_id'} })
		.exec(function (err, rev) {
	  		if (err) {
	  			...
	  			res.json({
	  				success: true,
	  				message: 'Your reviews has been retrieved successfully.',
	  				reviews: rev
	  			});
